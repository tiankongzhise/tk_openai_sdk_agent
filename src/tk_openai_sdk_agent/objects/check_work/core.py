...
from ...models import AgentSetting,VerifyResponse,VerifyRspResult,RspResult
        ...
        try:
            ...
            if isinstance(rsp_dict,list):
                for item in rsp_dict:
                    try:
                        response.append(VerifyResponse(**{**item,'ai_model':ai_model}))
                    except ValueError as e:
                        message.print(f"item is not a valid Response object:{item},error:{e}")
            elif isinstance(rsp_dict,dict):
                try:
                    response = [VerifyResponse(**{**rsp_dict,'ai_model':ai_model})]
                except ValueError as e:
            ...
            return VerifyRspResult(data=response,message="success",status="success")
        except Exception as e:
            return VerifyRspResult(fail_data=rsp+f'_@#ai_model={ai_model}',message=str(e),status="fail")
