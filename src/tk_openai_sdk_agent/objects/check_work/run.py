...
def get_before_second_dash(s: str) -> str:
    # 找第一个 '-' 的位置
    first_pos = s.find('-')
    if first_pos == -1:
        return s  # 无 '-'，返回原字符串
...
def find_missing_combinations(input_str_list:list[str], ai_model:str, db_client:Type[Curd]) -> list[str]:
    ...
    return [s for s in input_str_list if get_before_second_dash(s) not in existing_set]
...
async def run():
    ...
    source_data = source_data.fillna("未知")
    ip = source_data["IP"].tolist()
    character = source_data["角色"].tolist()
    deepseek_r1 = source_data["DEEPSEEK-R1"].tolist()
    doubao_thinking_pro = source_data["DOUBAO-THINKING-PRO"].tolist()
    doubao_vision_pro = source_data["DOUBAO-VISION-PRO"].tolist()
    deepseek_v3 = source_data["DEEPSEEK-V3"].tolist()
    ip_role_pairs = [f"{i}-{j}-{a1}-{a2}-{a3}-{a4}" for i, j,a1,a2,a3,a4 in zip(ip, character,deepseek_r1,deepseek_v3,doubao_thinking_pro,doubao_vision_pro)]

    target_ip_role_pairs = ip_role_pairs
    
    ai_models = ["DOUBAO-PRO-32K"]
    db_client = Curd()
