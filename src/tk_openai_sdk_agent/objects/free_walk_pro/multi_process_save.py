...
def local_xlsx_save(config: dict, rsp: Type[BaseDTO], path_time: str | None = None) -> None:
    import pandas as pd
    file_path = add_time_path(get_abs_file_path(config["LOCAL_XLSX_SAVE_FILE_PATH"]), path_time)
    file_name = create_file_name_by_rsp(file_path, rsp)
    df = pd.DataFrame([temp.model_dump() for temp in rsp.data])
...
class MultiProcessSave(object):
    ...
    def __init__(
        ...
        xlsx_save: Callable = local_xlsx_save,
        db_curd_class = Curd,
    ...
    def _run(self):
        ...
        while True:
            try:
                ...
                if item.status == "fail":
                    ...
                    message.info(f"成功保存{len(item.data)}条数据到txt本地文件")
                    
                    result_msg = ''
                    if self.config['IS_DB_SAVE']:
                        # 保存到数据库
                        db_save_result = self.db_save_func(item)
                        inserted_count += db_save_result
                        result_msg += f"成功保存{db_save_result}条数据到数据库,"
                    if self.config['IS_XLSX_SAVE']:
                        self.xlsx_save(self.config, item, self.runtime)
                        local_save_count += len(item.data)
                        result_msg += f"成功保存{len(item.data)}条数据到xlsx文件,"
                    message.info(result_msg)
            except Exception as e:
                # 处理异常
                message.error(f"数据保存过程中发生异常: {get_error_detail(e)},保存结果为{result_msg}")
                try:
                    # 尝试保存失败数据
        ...
        # 进程结束时输出统计信息
        message.info(
            f"从{self.runtime}开始多进程写入数据完毕，共数据库成功写入{inserted_count}条，本地xlsx写入{local_save_count}条,失败{fail_count}次"
        )
